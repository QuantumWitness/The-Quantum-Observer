...
# -*- coding: utf-8 -*-
"""
A script for a smol thermal machine
"""


# Let's try coupling 3 qubits

import numpy as np
import scipy.constants as scc
import matplotlib.pyplot as plt
from virtual_qubit_utils import calc_Tv,sim_3qubit
...
qubit_temps=[Ta, Tb, Tc]

max_time=100
num_timestep = 1001
times = np.linspace(0,max_time,num_timestep)

qubC_T, virtual_temperature, pops = sim_3qubit(Ea, Eb, Ec, Ta, Tb, Tc, g,
                                               max_time=max_time,
...
print(f'Qubit C Temp = {qubC_T*1000} mK')    
...
for j in range(np.size(pops,axis=1)):
...
# Now do lots of Eb values

Eb_list = np.logspace(0,2,26)*Ea


virtual_temps = np.zeros((len(Eb_list),))
qub_c_temps = np.zeros((len(Eb_list),))
for index,Eb in enumerate(Eb_list):
    
    Ec = Eb-Ea
    
    Tq, Tv, _ = sim_3qubit(Ea, Eb, Ec, Ta, Tb, Tc, g)
   
    qub_c_temps[index]=Tq
...
plt.semilogx(Eb_list/Ea,virtual_temps,'ro', markerfacecolor='none', markersize=10,
             label='T_virtual')
plt.semilogx(Eb_list/Ea, qub_c_temps,'k+',markersize=10,label='Tc')
plt.semilogx(Eb_theory/Ea,Tv,color='xkcd:periwinkle',linestyle='dashed',
             linewidth=3, label='Theory')
